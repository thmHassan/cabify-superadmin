...
const AddCompanyModal = ({
  ...
  const [initialValues, setInitialValues] = useState({
    ...
    map_provider: formData.map_provider || formData.maps_api || "mapify",
    search_provider: formData.search_provider || formData.search_api || "mapify",
    geocoding_provider: formData.geocoding_provider || formData.search_api || "mapify",
    routing_provider: formData.routing_provider || (formData.maps_api === "google" ? "google" : "barikoi"),
    credential_source: formData.credential_source || "platform",
    allow_platform_fallback: formData.allow_platform_fallback ?? false,
    google_browser_key: "",
    google_server_key: "",
    mapify_api_token: "",
    mapify_routing_url: "",
    ...
    billing_mode: formData.billing_mode || "one_time",
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        const subscriptionChanged =
          String(initialValues.subscription_type) !==
          String(values.subscription_type);
        const selectedDeductType = String(
          values.subscription?.deduct_type || ""
        ).toLowerCase();
        const requiresNewStripePayment =
          modalType === "company" &&
          type === "edit" &&
        ...
        if (modalType === "company") {
          ...
          // Stripe APIs require the tenancy primary key (for example,
          // "hassancompany10"), not the editable business company_id.
          const companyId =
            response.data.tenant_id ||
            (type === "edit" ? id : companyObj.id || response.data.data?.id);
